...
class KubernetesClient(object):
    ...
    def _run_command(self, cmd):
        return subprocess.check_output(
            " ".join(self._base_kubectl_command() + cmd), shell=True
        )

    def _run_command_and_parse_yaml(self, cmd):
        return yaml.load(self._run_command(cmd), yaml.CLoader)
    ...
    def get_entities(self, ns, entity):
        ...
        def cb():
            return self._get_entities(ns, entity)
    ...
    def get_object_in_format(self, ns, entity_type, object, format):
        ...
        def cb():
            return self._get_object_in_format(ns, entity_type, object, format)
    ...
    def describe(self, ns, entity_type, object):
        ...
        def cb():
            return self._describe(ns, entity_type, object)
    ...
    def _get_entities(self, ns, entity):
        payload = self._run_command_and_parse_yaml(
            ["get", entity, "-o", "yaml"] + self._namespace(ns)
        )
        if payload is None or "items" not in payload:
            return []
        return [item["metadata"]["name"] for item in payload["items"]]
    ...
    def _get_object_in_format(self, ns, entity_type, object, format):
        return self._run_command(
            ["get", entity_type, object, "-o", format] + self._namespace(ns)
        )

    def _describe(self, ns, entity_type, object):
        return self._run_command(
            ["describe", entity_type, object] + self._namespace(ns)
        )
